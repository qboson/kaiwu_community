...
if __name__ == "__main__":
    ...
    no_edges = [
        (u, v) for u in range(n) for v in range(n) if distance_matrix[u, v] == 0
    ]
    ...
    qubo_model.set_objective(
        kw.core.quicksum(
            [distance_matrix[u, v] * is_edge_used(path_matrix, u, v) for u, v in edges]
        )
    )

    # Node constraint: Each node must belong to exactly one position
    qubo_model.add_constraint(
        (path_matrix.sum(axis=0) - 1) ** 2 == 0, "sequence_cons", penalty=20
    )

    # Position constraint: Each position can have only one node
    qubo_model.add_constraint(
        (path_matrix.sum(axis=1) - 1) ** 2 == 0, "node_cons", penalty=20
    )

    # Edge constraint: Pairs without edges cannot appear in the path
    qubo_model.add_constraint(
        kw.core.quicksum([is_edge_used(path_matrix, u, v) for u, v in no_edges]) == 0,
        "connect_cons",
        penalty=20,
    )
