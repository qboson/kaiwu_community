...
import numpy as np
import kaiwu as kw
from kaiwu.core import Binary, get_val, get_val, get_sol_dict
...
def test_get_val_array():
    ...
    y_ising = kw.core.qubo_model_to_ising_model(qubo_model)
    vars = y_ising.get_variables()
    s = np.array([1, -1, 1, -1])
    sol_dict = get_sol_dict(s, vars)
    x_val = get_val(x, sol_dict)
    ans_x_val = np.array([[1.0, 0.0], [1.0, 0.0]])
