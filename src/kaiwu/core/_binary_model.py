...
class BinaryModel:
    ...
    def add_constraint(
        ...
        if isinstance(constraint_in, (list, tuple, np.ndarray)) or (
            isinstance(constraint_in, collections.abc.Iterable)
            and not isinstance(constraint_in, (str, bytes))
        ):
        ...
        if name is None:
            name = (
                f"constraint{len(self.hard_constraints) + len(self.soft_constraints)}"
            )
